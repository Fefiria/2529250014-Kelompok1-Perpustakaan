...
@section('content')
    <div class="page-content">
        <div class="card-body border-bottom border-secondary pb-3">
            <form action="{{ route('member.listbuku') }}" method="GET" id="form-filter-buku">
                    ...
                    <div class="col-md-3 col-12">
                        ...
                        <div class="d-flex gap-2 w-100">
                            ...
                            @if(request('search') || request('idGenre'))
                                <a href="{{ route('member.listbuku') }}" class="btn btn-danger text-white w-50 d-flex align-items-center justify-content-center gap-2">
                                ...
                                <div class="modal-body">
                                                        ...
                                                        <tr>
                                                            <td colspan="2" class="pt-3 pb-0">
                                                                <div class="rounded-4 p-3 border border-primary border-opacity-25" style="background-color: rgba(67, 94, 190, 0.08); width: 100%;">
                                                                    ...
                                                                    <p class="mb-0 text-primary text-opacity-75 small" style="text-align: justify; line-height: 1.6; max-height: 150px; overflow-y: auto;">
                                    ...
                                    <div class="col-12 mt-4 border-top border-secondary border-opacity-25 pt-4">
                                        <h6 class="mb-3 d-flex align-items-center gap-2" style="font-size: 1rem; font-weight: 600;">
                                            ...
                                            <span>Review & Ulasan Pembaca ({{ $buku->review_count ?? 0 }})</span>
                                        </h6>
                                                    ...
                                                    <div class="p-3 h-100 rounded-3" style="background-color: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.05);">
                                                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
                                                            <div class="d-flex align-items-center gap-2">
                                                                ...
                                                                <span class="fw-semibold small text-truncate" style="max-width: 80px;" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $review->nama }}">{{ $review->nama }}</span>
                                                                <span class="badge bg-light-primary text-primary" style="font-size: 0.6rem;">{{ ucfirst($review->role) }}</span>
                                                            </div>
                                            ...
                                            <div class="mt-4 pt-3 border-top border-secondary border-opacity-25">
                                                ...
                                                @if(!$reviewUser)
                                                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                                                        <span class="text-secondary small">Belum kasih penilaian? Yuk bagikan pendapatmu!</span>
                                                        <button class="btn btn-sm btn-primary d-inline-flex align-items-center gap-2" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFormReview-{{ $buku->idBuku }}" aria-expanded="false">
                                                            <i class="bi bi-pencil-square" style="line-height: 1; font-size: 0.95rem;"></i> <span>Tulis Ulasan Kamu</span>
                                                        </button>
                                                    </div>
                                                    ...
                                                    <div class="collapse" id="collapseFormReview-{{ $buku->idBuku }}">
                                                        <div class="card card-body mt-3 p-3 text-white border-secondary" style="background-color: rgba(255, 255, 255, 0.02);">
                                                            <form action="{{ route('member.review.store', $buku->idBuku) }}" method="POST">
                                                                @csrf
                                                                <div class="row g-3">
                                                ...
                                                @else
                                                    <div class="p-3 rounded-3 border border-success border-opacity-25 d-flex justify-content-between align-items-center flex-wrap gap-3" style="background-color: rgba(25, 135, 84, 0.04);">
                                                        <div>
                                                            <span class="badge bg-success mb-1"><i class="bi bi-check-circle-fill me-1"></i> Sudah Direview</span>
                                                            <p class="mb-0 text-secondary small">Kamu sudah mereview buku ini dengan skor <strong class="text-warning">★ {{ $reviewUser->pivot->rating }}</strong></p>
                                                        </div>
                                                        ...
                                                        <div class="d-flex gap-2">
                                                            ...
                                                            <form action="{{ route('member.review.destroy', $reviewUser->pivot->idReview) }}" method="POST" onsubmit="displayAlert(event, this, '{{ 'Apakah anda ingin menghapus review untuk buku ' . $buku->judul . ' ?' }}', 'warning')">
                                                                @csrf
                                                                @method('DELETE')
                                                    ...
                                                    <div class="collapse" id="collapseEditReview-{{ $buku->idBuku }}">
                                                        <div class="card card-body mt-3 p-3 text-white border-secondary" style="background-color: rgba(255, 255, 255, 0.02);">
                                                            <form action="{{ route('member.review.update', $reviewUser->pivot->idReview) }}" method="POST">
                                                                @csrf
                                                                @method('PUT')
                                                                <div class="row g-3">
                                                                    <div class="col-12 col-md-4 border-end-md border-secondary border-opacity-25 text-center text-md-start pt-2">
